  ...
  $effect(() => {
    ...
    ;(async () => {
      ...
      const { data: eventData } = await supabase
        .from('events')
        .select('id, title, event_date, location, status, cover_photo_url')
        .eq('group_id', groupId)
        .eq('status', 'confirmed')
...
<div class="max-w-2xl mx-auto px-4 py-6">
  ...
  {:else}
    ...
    <!-- Next event -->
    {#if nextEvent}
      <div class="rounded-xl p-4 mb-4" style="background: hsl(234 20% 89%)">
        <div class="flex items-start gap-3">
          <!-- Date circle or cover photo -->
          <div class="w-11 h-11 rounded-full shrink-0 overflow-hidden" style="background: hsl(220 14% 93%)">
            {#if nextEvent.cover_photo_url}
              <img src={nextEvent.cover_photo_url} alt={nextEvent.title} class="w-full h-full object-cover" />
            {:else}
              <div class="w-full h-full flex flex-col items-center justify-center gap-0 select-none">
                <span class="font-semibold uppercase tracking-widest leading-none" style="font-size: 8px; color: hsl(220 9% 65%)">{new Date(nextEvent.event_date).toLocaleDateString('en-US', { month: 'short' })}</span>
                <span class="font-bold leading-none mt-0.5" style="font-size: 1.25rem; color: hsl(220 9% 50%)">{new Date(nextEvent.event_date).getDate()}</span>
              </div>
            {/if}
          </div>

          <div class="flex-1 min-w-0">
            <p class="text-xs font-semibold uppercase tracking-wider mb-0.5" style="color: hsl(234 12% 52%)">Next event</p>
            <p class="text-base font-bold leading-snug text-foreground">{nextEvent.title}</p>
            <p class="text-sm mt-0.5 text-muted-foreground">{formatEventDate(nextEvent.event_date)} at {formatEventTime(nextEvent.event_date)}</p>
            {#if nextEvent.location}
              <p class="text-sm text-muted-foreground">{nextEvent.location}</p>
            {/if}
          </div>

          <a href="/groups/{groupId}/events/{nextEvent.id}" class="shrink-0 mt-1">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="hsl(234 12% 52%)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    {:else}
      <div class="flex flex-col gap-2">
        {#each recentPosts as post}
          <div class="rounded-xl bg-white p-4 hover:bg-muted/30 transition-colors cursor-pointer" onclick={() => goto(`/groups/${groupId}/posts/${post.id}`)}>
