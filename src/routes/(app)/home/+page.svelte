<script>
  ...
  onMount(async () => {
    ...
    const { data: newEvents } = await supabase
      .from('events')
      .select('id, title, status, cover_photo_url, created_at, group_id, host_user_id, groups(name), users(display_name, avatar_url)')
      .in('group_id', groupIds)
      .gte('created_at', weekAgo)
      .order('created_at', { ascending: false })
      .limit(5)
    ...
    const eventItems = (newEvents ?? []).map(e => ({
      ...
      eventCoverPhoto: e.cover_photo_url ?? null,
      time: e.created_at,
    }))
